...
The base r function as.formula is not very good. It can only handle very basic strings. For instance 
```{r iris}
as.formula("money ~ brains + looks") # works
```
but
```{r error=TRUE}
as.formula("money ~ net-worth + tips") # doesn't work
```
because it can't recognise that net-worth is a single word. 
...
as_formula("money ~ net-worth + tips")
```
