---
title: 'metaprogramming with map(quo) %>% reduce '
...
Ever since I discovered the rlang function 'expr' I have become mildy obsessed with the idea of writing code which writes code (I think this falls under the banner of metaprogramming, hence the title of this post). Before launching into anything genuinely useful, some background is required: R is an 'eagerly evaluated' language, which means if you write:
```{r}
a <- 50
```
...
a
```
This become problematic when you want one variable, let's called it a, to hold the name of
another variable, b. If you write:
```{r error=TRUE}
a <- b
```
You get an error, because r is eagerly trying to evaluate b, so that is can pass b's value to a. That's not r's fault, that's just the way it was built. 
